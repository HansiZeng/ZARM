...
class SelfMultiheadAtt_old(nn.Module):
    def __init__(self, in_feat, out_feat, kernel_size, num_head, dropout):
        super().__init__()
    ...
    def forward(self, inputs, key_padding_mask=None):
        ...
        """
        bz, seq_len, _ = inputs.size()

        inputs = inputs.permute(0,2,1) #[bz, in_feat, seq_len]
        ...
        outputs = self.concat_heads(outputs) #[bz, seq_len, out_feat]
...
class NgramFeatWithSeq(nn.Module):
    def __init__(self, in_feat, out_feat, kernel_size, num_head, dropout):
        super().__init__()
        
        self.multihead_att_layer = SelfMultiheadAtt(in_feat, out_feat, kernel_size, num_head, dropout)
        self.add_att_layer = AddictiveAttention(out_feat, out_feat//2)

    def forward(self, inputs, input_masks):
        """
        Args:
            inputs: [bz, seq_len, out_feat]
            input_masks: [bz, seq_len]

        Returns:
            ouputs: [bz, out_feat]
            ngrams: [bz, seq_len, out_feat]
        """
        ngrams = self.multihead_att_layer(inputs)
        outputs, _ = self.add_att_layer(ngrams, input_masks)

        return outputs, ngrams
...
class LastFeat(nn.Module):
    ...
    def forward(self, text_feat, my_id):
        ...
        #out_feat = F.relu(text_feat @ self.W + self.b) + self.ebd(my_id)
