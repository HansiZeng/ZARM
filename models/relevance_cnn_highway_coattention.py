...
from .layers import UserCoRelLogitWithAggre, CosineInteraction, WordScore, SingleRelLogit, ZeroAttention, FMWithoutUIBias
...
class RelevanceCNNHighwayCoAttention(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, kernel_sizes, latent_dim,
                vocab_size, user_size, item_size, rv_len,
                pretrained_embeddings, dropout, pooling_mode, sparse, word_dropout, mode, temperature, arch=None, use_ui_bias=True):
        super().__init__() 
        ...
        if "user" in self.mode or "item" in self.mode:
            ...
            self._rel_score_layer = nn.Sequential(nn.Dropout(p=0.1),
                                                ...
                                                nn.Tanh(),
                                                nn.Linear(16, 1))
            self.user_rel_logit_layer = UserCoRelLogitWithAggre(self.seq_dim, rv_len, 
                        cosine_interaction=self._cosine_interaction, word_score_layer=self._word_score_layer, 
                        rel_score_layer=self._rel_score_layer, temperature=temperature)
    ...
    def forward(self, u_revs, i_revs, u_rev_word_masks, i_rev_word_masks, u_rev_masks, i_rev_masks, u_ids, i_ids,
        ...
        if "user" in self.mode:
            ...
            u_rel_logits, u_rel_revs = self.user_rel_logit_layer(u_rev_seqs, i_rev_seqs, u_rev_word_masks, i_rev_word_masks) #[bz, rv_num]
...
class RelevanceCNNHighwayV1(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, kernel_sizes, latent_dim,
                vocab_size, user_size, item_size, rv_len,
                pretrained_embeddings, dropout, pooling_mode, sparse, word_dropout, mode, temperature, arch=None):
        super().__init__() 
        self.embedding_dim = embedding_dim
        self.hidden_dim = hidden_dim
        self.mode = mode
