...
class NarreExperiment(Experiment):
    ...
    def build_model(self):
        from models.relevance_hsan_highway import RelevanceHsanHighway
        ...
        self.model = RelevanceHsanHighway(embedding_dim=self.args.embedding_dim, hidden_dim=self.args.hidden_dim,
                                    kernel_size=self.args.kernel_size, latent_dim=self.args.latent_dim,
                                    vocab_size=len(_dataset.word_vocab), user_size=_dataset.user_num,
                                    item_size=_dataset.item_num, rv_len=_dataset.rv_len,
                                    pretrained_embeddings=word_pretrained, dropout=self.args.dropout, 
                                    sparse=self.args.sparse, word_dropout=self.args.word_dropout, mode=self.args.mode)
    ...
    def train_one_epoch(self, current_epoch):
        ...
        self.model.train()
        for i,  ((u_ids, i_ids, ratings), (u_revs, i_revs, u_rev_word_masks, i_rev_word_masks, u_rev_sent_masks, i_rev_sent_masks, 
            ...
            if i == 0 and current_epoch == 0:
                ...
                #print("u_text:", u_revs[:2], u_rev_word_masks[:2], u_rev_sent_masks[:2], u_rev_masks[:2])
                #print("i_text:", i_revs[:2], i_rev_word_masks[:2], i_rev_sent_masks[:2], i_rev_masks[:2])
            ...
            u_rev_sent_masks = u_rev_sent_masks.to(self.device)
            i_rev_sent_masks = i_rev_sent_masks.to(self.device)
            ...
            y_pred, (gt_logits, ng_logits), (rel_feat_norm, zero_perc) = \
                                self.model(u_revs, i_revs, u_rev_word_masks, i_rev_word_masks, u_rev_sent_masks, i_rev_sent_masks,
                                            u_rev_masks, i_rev_masks, u_ids, i_ids, ui_revs, neg_ui_revs, ui_word_masks, neg_ui_word_masks, 
                                            training=True)
            ...
            loss = loss_1 + self.args.reg_weight * loss_2 
            loss.backward()
            ...
            if (i+1) % self.args.log_idx == 0 and self.args.log:
                ...
                """
                print("gt_logits", np.quantile(gt_logits.cpu().data.numpy(), [0.1, 0.5, 0.9]))
                print("ng_logits", np.quantile(ng_logits.cpu().data.numpy(), [0.1, 0.5, 0.9]))
    ...
    def valid_one_epoch(self):
        ...
        self.model.eval()
        for i,  ((u_ids, i_ids, ratings), (u_revs, i_revs, u_rev_word_masks, i_rev_word_masks, u_rev_sent_masks, i_rev_sent_masks, 
                u_rev_masks, i_rev_masks), (u_rids, i_rids)) in enumerate(self.valid_dataloader):
            u_ids = u_ids.to(self.device)
            i_ids = i_ids.to(self.device)
            ratings = ratings.to(self.device)
            u_revs = u_revs.to(self.device)
            ...
            with torch.no_grad():    
                y_pred, _= self.model(u_revs, i_revs, u_rev_word_masks, i_rev_word_masks, u_rev_sent_masks, i_rev_sent_masks, 
                                    u_rev_masks, i_rev_masks,  u_ids, i_ids, ui_rev=None, neg_ui_rev=None, ui_mask=None, 
                                    neg_ui_mask=None, training=False)
        ...
        log_text =  "valid rmse: {:.3f}, best rmse: {:.3f}".format(rmse, self.best_rmse)
        self.print_write_to_log(log_text)
...
class NarreDatasetSameUIReviewNum(torch.utils.data.Dataset):
    ...
    @staticmethod
    def get_rev_word_mask(inputs):
        """
        Args:
            inputs: [bz, rv_num, sent_num, word_num]

        Returns:
            masks: [bz, rv_num, sent_num, word_num]
        """
        bz, rv_num, sent_num, word_num = inputs.size()

        masks = torch.ones(size=(bz, rv_num, sent_num, word_num)).int()
        masks[inputs==0] = 0

        return masks.bool()
    
    @staticmethod
    def get_rev_sent_mask(inputs):
        """
        Args:
            inputs: [bz, rv_num, sent_num, word_num]
        
        Returns:
            masks: [bz, rv_num, sent_num]
        """
        bz, rv_num, sent_num, _ = inputs.size()

        masks = torch.ones(size=(bz, rv_num, sent_num)).int()
        inputs = inputs.sum(dim=-1) #[bz, rv_num, sent_num]
        masks[inputs==0] = 0
    ...
    def get_rev_mask(inputs):
        ...
        Args:
            inputs: [bz, rv_num, sent_num, word_num]
        """
        bz, rv_num, _, _ = list(inputs.size())

        masks = torch.ones(size=(bz, rv_num)).int()
        inputs = inputs.sum(dim=(-2,-1)) #[bz, rv_num]
        masks[inputs==0] = 0 

        return masks.bool()

    def get_all_masks(self, inputs):
        return self.get_rev_word_mask(inputs), self.get_rev_sent_mask(inputs), self.get_rev_mask(inputs)
    ...
    def train_collate_fn(self, batch):
        ...
        u_rev_word_masks, u_rev_sent_masks, u_rev_masks = self.get_all_masks(u_revs)
        i_rev_word_masks, i_rev_sent_masks, i_rev_masks = self.get_all_masks(i_revs)
...
if __name__ == "__main__":
    config_file = "./models/hzarm_train.json"
